...
def get_financial_data(ticker):
    ...
    try:
        ...
        if not actual_data.empty:
            ...
            if result['Predicted Dividend Amount'] != "N/A" and result['Actual Dividend'] != "N/A":
                result['Dividend Prediction Error'] = abs(float(result['Predicted Dividend Amount']) - float(result['Actual Dividend']))
                result['Prediction Accuracy (%)'] = 100 - (abs(float(result['Predicted Dividend Amount']) - float(result['Actual Dividend'])) / float(result['Actual Dividend']) * 100
            else:
        ...
        st.error(f"Error loading actual dividend data: {e}")
        result['Actual Dividend'] = "N/A"
        result['Actual Ex Date'] = "N/A"
        result['Dividend Prediction Error'] = "N/A"
        result['Prediction Accuracy (%)'] = "N/A"
...
def save_to_excel(results, filename="dividend_predictions.xlsx"):
    try:
        ...
        if 'Past Dividends' in results_df.columns:
            results_df['Past Dividends'] = results_df['Past Dividends'].apply(lambda x: ', '.join(map(str, x)) if isinstance(x, list) else x)
    ...
    if 'Symbol' not in symbols_df.columns:
        ...
        if st.button('Fetch Financial Data') and selected_stocks:
            ...
            if all_results:
                ...
                valid_errors = results_df[(results_df['Dividend Prediction Error'] != "N/A") & 
                                        (results_df['Actual Dividend'] != "N/A")]
                
                if not valid_errors.empty:
                    st.write(f"### Prediction Accuracy Summary")
                    st.write(f"Number of stocks with valid predictions: {len(valid_errors)}")
                    st.write(f"Mean Absolute Error: {valid_errors['Dividend Prediction Error'].mean():.2f}")
                    st.write(f"Average Prediction Accuracy: {valid_errors['Prediction Accuracy (%)'].mean():.2f}%")
